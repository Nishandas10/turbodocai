...
class QueryService {
     ...
     */
    async getFullRawText(userId, documentId, maxWaitMs = 60000) {
        var _a, _b, _c;
        const db = (0, firestore_1.getFirestore)();
                ...
                if (snap.exists) {
                    ...
                    if (transcriptPath) {
                        try {
                            ...
                            firebase_functions_1.logger.debug("getFullRawText: transcript read not ready, retrying", e);
                        }
                    }
                }
            }
            catch (e) {
                firebase_functions_1.logger.debug("getFullRawText polling failed (continuing)", e);
            }
...
${documentContent}`;
            const resp = await this.openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are an expert analyst and note-taking system. Produce long, detailed, well-structured markdown notes with comprehensive coverage. Be faithful to the source content and do not add information not present in the document.",
                    },
     ...
     */
    async generateDocumentSummary(documentId, userId, maxLength = 4000) {
        try {
            ...
            try {
                ...
                if (snap.exists) {
                    ...
                    let fullContent = await this.getFullRawText(userId, documentId, 30000);
                    if (fullContent && fullContent.length > 100) {
                        firebase_functions_1.logger.info("Attempting summary with FULL document content", {
                            documentId,
                            contentLength: fullContent.length,
                        });
                        try {
                            // Try with full content first
                            const summary = await this.summarizeWithSpecificContent(fullContent, // Use complete content without truncation
                            maxLength, docTitle);
                            if (summary && summary.trim().length) {
                                firebase_functions_1.logger.info("Successfully generated summary with full content", {
                                    documentId,
                            ...
                            firebase_functions_1.logger.warn("Full content summarization failed, trying truncated version", {
                                error: fullError,
                            ...
                            try {
                                ...
                                firebase_functions_1.logger.info("Retrying with truncated content (18k chars)", {
                                    documentId,
                                    truncatedLength: truncatedContent.length,
                                });
                                const summary = await this.summarizeWithSpecificContent(truncatedContent, maxLength, docTitle);
                                if (summary && summary.trim().length) {
                                    firebase_functions_1.logger.info("Successfully generated summary with truncated content", {
                                        documentId,
                                        summaryLength: summary.length,
                                    });
                                    return summary;
                                ...
                                firebase_functions_1.logger.warn("Truncated content summarization also failed", truncatedError);
                            }
                        }
                    }
                }
            ...
            firebase_functions_1.logger.info("Using final fallback path", { documentId });
            // First try full text
            ...
            if (!fallbackText || fallbackText.length < 80) {
                ...
                firebase_functions_1.logger.warn("Full text unavailable in fallback, trying truncated", {
                    documentId,
                });
            ...
            firebase_functions_1.logger.info("Generating summary with fallback content", {
                documentId,
            ...
            return await this.summarizeWithSpecificContent(fallbackText, maxLength, process_1.title // We don't have title here, but it's fine
            );
        }
        catch (error) {
