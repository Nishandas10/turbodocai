...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#fbf9f7] flex flex-col items-center justify-center p-4 relative">
      <div className="absolute inset-0 bg-gradient-to-br from-[#fbf9f7] via-white to-[#fbf9f7]"></div>
  ...
  <div className="bg-white rounded-xl p-8 border border-black/10 relative">
          ...
          {message && (
            <div className="mb-4 p-3 rounded bg-red-50 border border-red-200 text-red-700 text-sm">{message}</div>
          )}
          ...
          <form onSubmit={handleLogin} className="space-y-4 mb-6">
            ...
            <Button
              type="submit"
              className="w-full h-11 bg-[#F7D978] hover:bg-[#f5cf60] text-[#1A1A1A] font-medium disabled:opacity-50 disabled:cursor-not-allowed border border-black/10"
              disabled={isLoading}
            >
              {isLoading ? (
                <div className="w-4 h-4 border-2 border-[#1A1A1A]/70 border-t-transparent rounded-full animate-spin mr-2" />
              ) : null}
              Sign in
            </Button>
          </form>
